define(["require", "exports", "./displayed_films", "./film_array", "jquery"], function (require, exports, displayed_films_1, film_array_1, $) {
    ...
    var dFilms = new displayed_films_1.DisplayedFilms();
    var selectedFilms = new displayed_films_1.DisplayedFilms();
    var filmArr = new film_array_1.FilmArray();
    ...
    function generateMovieTable() {
        ...
        var tableBody = $("tbody");
        for (var i = 0; i < filmArr.films.length; i++) {
            var currentFilm = filmArr.films[i];
            ...
            expandTd.addEventListener("click", function (e) { expandDetails(this, e); }, false);
    ...
    function orderColumn(th, e) {
        if (columnSort.sortedIdx !== th.cellIndex) {
    ...
    function selectRow(elem) {
        if (!$(elem).hasClass("selected")) {
            ...
            var entry = dFilms.getEntryByRow(elem);
            if (entry !== null)
    ...
    function expandDetails(elem, e) {
        ...
        if (previousExpanded !== $(elem).parent()[0]) {
            ...
            var selectedEntry = dFilms.getEntryByRow(selectedRow[0]);
    ...
    function onSubmitSearch() {
        ...
        if (inputValue !== undefined) {
            ...
            var matches = dFilms.searchString(val, false);
            var trs = $("tbody tr");
